...
async function starts() {
	...
	nzwa.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('Escanea el código pero rapido puto'))
	})
	...
	await nzwa.connect({timeoutMs: 30*1000})
        ...
        lolcatjs.fromString('[Josue] Hola gracias por usar mi bot, espero te guste ;)')
		...
		try {
			...
			const isCmd = body.startsWith(prefix)
                        ...
                        mess = {
							...
							error: {
								      stick: '[❗] No se pudo convertir a sticker, asegurate de que el video sea menos de 10 seg y si es foto vuelve a intentar o no vuelvas a joder puto❌',
								    Iv: '❌ Link invalido ❌'
							},
							only: {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '593992390339@s.whatsapp.net'
